...
pub fn boolean(from: bool) -> Value { Value::Boolean(from) }
pub fn integer(from: i64) -> Value { Value::Integer(from) }
pub fn float(from: f64) -> Value { Value::Float(from) }
...
impl Collection
{
    pub fn size(&self) -> Value
    {
        match &self.contents
        {
            CollectionContents::Zero => integer(0),
            CollectionContents::One(_) => integer(1),
...
impl CollectionBuilder
{
    ...
    pub fn push(mut self, value: Value) -> PureExecutionResult<Self>
    {
...
mod tests
{
    ...
    fn build_many_integer() -> PureExecutionResult<()>
    {
        ...
        for i in 0..50
        {
            builder = builder.push(integer(i))?;
        }
        ...
        for i in 0..50
        {
            assert_eq!(Some(&integer(i)), iter.next());
        }
        assert_eq!(None, iter.next());
    ...
    {
        let numbers = CollectionBuilder::new(Type::Number, ZERO_MANY).push(integer(1))?.push(float(2.0))?.build()?;

        assert_eq!(2, numbers.size().as_i64()?);
        assert_eq!(pure_type::Type::Number, numbers.pure_type());

        let mut iter = (&numbers).into_iter();
        assert_eq!(Some(&integer(1)), iter.next());
    ...
    {
        let bad = CollectionBuilder::new(Type::Integer, ZERO_ONE).push(float(2.0));
    ...
    {
        let bad = CollectionBuilder::new(Type::Integer, PURE_ONE).push(integer(1))?.push(integer(1));
    ...
    {
        let bad = CollectionBuilder::new(Type::Integer, 2.into()).push(integer(1))?.build();
